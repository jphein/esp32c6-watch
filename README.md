...
- **Watch-to-watch ping** — a hero button greets the other watch by sigil over the mesh (additive SMOLv1 `PING`/`PINGACK` frames with delivery confirmation). The receiver wakes from AOD, blooms a full-screen accent-ring pulse naming the sender, and plays a rising two-tone chime; if the panel is off or a game holds the framebuffer, it gets the chime plus a notification-shade card.
...
ui/slint/         the Slint scene: shell.slint, controls.slint (shared components),
                  ...
                  switcher, shade, power_menu, ping, climate, energy, lights, voice,
...
- *v0.11.0* — **press-once voice** ([#22](https://github.com/jphein/esp32c6-watch/issues/22): an early PTT press latches and auto-fires when the link lands) and **watch-to-watch ping** ([#35](https://github.com/jphein/esp32c6-watch/issues/35): mesh `PING`/`PINGACK`, a sigil-named full-screen pulse and two-tone chime on the receiver). `mythic-throne` took this release **over the air, zero-touch** — the payoff for the net_task, the slot guard, and roaming all landing together.
...
- [#57](https://github.com/jphein/esp32c6-watch/issues/57) — **Firmware WiFi roaming** — best-BSSID select + RSSI reassociation *(core landed in v0.10.1; issue still open for follow-up)*
