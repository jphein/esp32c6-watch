...
fn ieee802154_txpower_convert(txpower: i8) -> u8 {
    // esp32c6 TXPOWER limits (this vendor is esp32c6-only, so the esp-radio
    // per-chip cfg_if! — which relied on esp-metadata's bare `esp32c6` cfg not
    // set in our build — is inlined). esp-idf v5.3 hal/esp32c6 ieee802154_ll.h.
