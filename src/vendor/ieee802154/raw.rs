...
use super::clocks::{enable_ieee802154, init_radio_clocks};
...
unsafe extern "C" {
    ...
    // Coex PTI / break-notify externs (esp_coex_ieee802154_*) stripped — the
    // passive-scan vendor tears wifi down before scanning, so there is no
    // wifi/154 coex to arbitrate. (bt_bb_* stay — needed for baseband init.)
}
...
fn ieee802154_mac_init(radio: IEEE802154<'_>) {
    // (coex_pti_tab pointer setup stripped — no wifi/154 coex in scan mode.)
...
fn ieee802154_set_txrx_pti(_txrx_scene: Ieee802154TxRxScene) {
    // Coex PTI stripped for the passive-scan vendor (wifi down during scan → no
    // wifi/154 coex arbitration). Kept as a no-op so call sites stay valid.
}
...
fn isr_handle_rx_done(needs_next_op: &mut bool) {
    event_end_process();
    unsafe {
        // esp-radio wrapped this in crate::fmt::Bytes for a hex view; log the
        // raw array directly (the fmt::Bytes helper isn't part of the vendor).
        trace!("Received raw {:?}", &*core::ptr::addr_of!(RX_BUFFER));
...
fn isr_handle_tx_abort(tx_abort_reason: u32, needs_next_op: &mut bool) {
    ...
    match tx_abort_reason {
        ...
        r if r == TxAbortReason::TxCoexBreak as u32 => {
            // coex break-notify stripped (no wifi/154 coex in scan mode)
            super::tx_failed();
            *needs_next_op = true;
        }
        // TX security error
