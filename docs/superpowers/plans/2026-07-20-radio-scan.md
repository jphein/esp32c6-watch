...
**esp-radio 0.18 will not compile `ieee802154` together with `wifi`.** Its build
script aborts unconditionally — `esp-radio-0.18.0/build.rs:40-41`:
...
The 802.15.4 driver is **in-tree to esp-radio** (`src/ieee802154/`, drives
`esp_hal::peripherals::IEEE802154`) — **no standalone `esp-ieee802154` crate**
exists to sidestep the gate. So **one firmware image cannot hold both the WiFi
and 802.15.4 drivers**, which **invalidates RS0/RS3's runtime radio-switch**:
"drop `wifi_controller`, then `Ieee802154::new()`" can never link (compile-time
mutual exclusion, not a runtime PHY-share). Not a RAM issue; not config/coex
fixable (only `ble + ieee802154 + coex` is allowed).

**Option space (JP product decision before RS3+):** (1) two firmware images,
build-time wifi↔ieee802154 swap, enter-scan = reboot into the scan image (heavy;
reboot-to-scan UX needs sign-off); (2) drop WiFi entirely (kills ESP-NOW mesh +
NTP — unacceptable); (3) redesign / defer.

**Status of the tasks under this constraint:**
- **RS1** (add the esp-radio `ieee802154` feature): ❌ **BLOCKED** — does not
  compile alongside `wifi` (this section is the finding). Baseline sizes before
  the attempt: esp32c6-watch text 2,518,796 / data 20,532 / bss 356,008.
- **RS0, RS3, RS4, RS6**: on hold — all assume the single-image runtime switch.
- **RS2** (`crates/scan-model` parser + model): ✅ DONE, 11 host tests,
  oracle-cleared, **image-agnostic** — reusable under any chosen strategy.
- **RS5** (Slint scan overlay UI): **image-agnostic** — no rework needed.

Everything below is the *intended* runtime behavior for whichever image strategy
is chosen; it does not describe a buildable single-image design today.
