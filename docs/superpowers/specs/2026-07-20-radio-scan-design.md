...
## ⚠ HARD CONSTRAINT — single-image WiFi↔802.15.4 is INFEASIBLE (found RS1, 2026-07-20)

**esp-radio 0.18 refuses to compile `ieee802154` together with `wifi`.** Its
build script aborts unconditionally — `esp-radio-0.18.0/build.rs:40-41`:

```rust
if cfg!(feature = "ieee802154") && (cfg!(feature = "wifi") || cfg!(feature = "wifi-eap")) {
    panic!("802.15.4 and Wi-Fi won't work together");
}
```

The 802.15.4 driver lives **inside** esp-radio (`src/ieee802154/`, driving
`esp_hal::peripherals::IEEE802154`) — there is **no standalone `esp-ieee802154`
crate** to route around the gate. Consequence: **one firmware image cannot
contain both the WiFi driver and the 802.15.4 driver.**

This **invalidates the runtime radio-switch design** below (the "Radio
lifecycle" crux; plan RS0 spike + RS3 lifecycle refactor). "Drop
`wifi_controller` at runtime, then `Ieee802154::new()`" assumes both drivers
co-reside in one binary and you flip between them at runtime — but they are
mutually exclusive at **compile** time, so the drop-and-recreate can never link.
It is **not a RAM problem** (the half-res framebuffer does not help) and **not
fixable by config/coex** (the gate is flat; only `ble + ieee802154 + coex` is
permitted, never `wifi + ieee802154`).

### Option space (needs a JP product decision before RS3+)
1. **Two firmware images** — a build-time `radio-scan` feature swapping
   wifi↔ieee802154. Entering scan = OTA/reboot into the scan image, reboot back.
   Heavy: dual images, OTA dance, partition budget; RS0's "reboot-to-restore"
   fallback becomes "reboot-to-**switch-firmware**". Reboot-to-scan is a UX
   change JP must sign off.
2. **Drop WiFi from the firmware entirely** — kills ESP-NOW mesh + NTP +
   weather. Almost certainly unacceptable.
3. **Redesign / defer** — the C6's single 2.4 GHz PHY can't run WiFi + 802.15.4
   concurrently; esp-radio enforces it at build. (A second-opinion pass is
   confirming there's truly no single-image path — raw peripheral / OpenThread /
   other crate — before escalation.)

### What survives regardless of the chosen strategy
- **RS2** — `crates/scan-model` (802.15.4-2006 MHR parser + PAN/channel model,
  11 host tests, oracle-cleared): pure and **image-agnostic**, reusable under
  any strategy. DONE.
- **RS5** — the Slint scan-overlay UI: **image-agnostic**, no rework needed.

Both are decoupled from the radio-lifecycle question. The rest of this spec's
runtime-switch architecture is on hold pending the option-1/2/3 decision.
...
> ⚠ **INFEASIBLE as written** — see "HARD CONSTRAINT" above. `Ieee802154::new()`
> (step 4) needs esp-radio's `ieee802154` feature, which will not compile
> alongside `wifi`. The runtime drop-and-recreate below cannot link in a single
> image; it stands only as the *intended* behavior for whichever image strategy
> is chosen (e.g. it becomes the scan-image's boot path under option 1).
