...
/// [esp32c6-watch LOCAL PATCH — not upstream Slint 1.17.1]
///
/// Expand every rectangle of a dirty region outward to the even-pixel grid.
///
/// CO5300/SH8601-class QSPI AMOLED controllers restrict CASET (2Ah) / RASET
/// (2Bh): the start coordinate AND the extent must both be divisible by 2, on
/// BOTH axes (CO5300 datasheet V0.00 §7.5.21/§7.5.22). A line-streaming
/// `LineBufferProvider` with no full framebuffer can only flush even-aligned
/// windows, and every pixel it flushes must have been freshly rendered this
/// frame (there is no framebuffer to re-send stale neighbours from). Aligning
/// the dirty region BEFORE item filtering and span emission guarantees both:
/// the renderer paints the (at most 1px) expansion borders itself, and dirty
/// lines always arrive in complete even/odd row pairs with identical span
/// ranges (see `region_line_ranges`: range sets change only at rect y-edges,
/// which this makes even). This is the exact equivalent of the even-alignment
/// "rounder" callback the vendor firmware registers with LVGL for this panel.
///
/// `add_box` may merge expanded boxes that now touch; the union of even-grid
/// boxes is still even-grid, so the guarantee survives merging.
fn align_dirty_region_to_even_grid(region: &DirtyRegion) -> DirtyRegion {
    // f32 floor/ceil to the even integer grid without std/libm float methods.
    // Exact for |v| < 2^23 — far beyond any physical screen coordinate.
    fn floor_even(v: f32) -> f32 {
        let mut i = v as i32; // truncation toward zero
        if (i as f32) > v {
            i -= 1; // true floor for negative fractions
        }
        (i & !1) as f32
    }
    fn ceil_even(v: f32) -> f32 {
        let mut i = v as i32;
        if (i as f32) < v {
            i += 1; // true ceil for positive fractions
        }
        ((i + 1) & !1) as f32
    }
    let mut out = DirtyRegion::default();
    for b in region.iter() {
        if b.is_empty() {
            continue;
        }
        out.add_box(euclid::Box2D::new(
            euclid::point2(floor_even(b.min.x), floor_even(b.min.y)),
            euclid::point2(ceil_even(b.max.x), ceil_even(b.max.y)),
        ));
    }
    out
}
    ...
    fn render_buffer_impl(
        ...
        window_inner
            .draw_contents(|components, post_render| {
                ...
                // [esp32c6-watch LOCAL PATCH] CO5300 CASET/RASET windows must be
                // even-aligned with even extents on both axes; align the region
                // before item filtering so expansion pixels render this frame.
                renderer.dirty_region = align_dirty_region_to_even_grid(&renderer.dirty_region);
...
fn prepare_scene(
    ...
    window.draw_contents(|components, post_render| {
        ...
        // [esp32c6-watch LOCAL PATCH] CO5300 CASET/RASET windows must be
        // even-aligned with even extents on both axes; align the region before
        // item filtering and line-span emission so that (a) every span handed to
        // process_line is even-aligned and (b) dirty lines arrive in complete
        // even/odd row pairs — the TwoLineFlusher's correctness contract.
        renderer.dirty_region = align_dirty_region_to_even_grid(&renderer.dirty_region);
